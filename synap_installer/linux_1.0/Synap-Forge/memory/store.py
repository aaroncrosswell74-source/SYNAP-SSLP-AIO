...
class MemoryStore:
    ...
    def add_memory(self, content: str, memory_type: str = "interaction", 
        ...
        try:
            ...
            file_path = interactions_dir / f"{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}.json"
    ...
    def get_recent(self, limit: int = 10, user_id: str = "default") -> List[Dict]:
        """Get recent memories from Redis, falling back to JSON if unavailable."""
        results = []
        ...
        if self.redis:
            try:
                ...
                logger.debug(f"Redis recent-memory read failed: {e}")

        # 2. FALLBACK: JSON (mirroring search() behavior)
        try:
            interactions_dir = self.central_memory / "interactions"
            ...
            if interactions_dir.exists():
                ...
                for file_path in files[:limit]:
                    try:
                        with open(file_path, "r") as f:
                            data = json.load(f)
                            # Convert JSON format to match Redis format
                            results.append({
                                "role": "assistant" if data.get("type") == "response" else "user",
                                "content": data.get("content", ""),
                                "timestamp": data.get("timestamp", "")
                            })
                    except Exception as e:
                        logger.debug(f"Failed reading memory file {file_path}: {e}")

        except Exception as e:
            logger.debug(f"JSON recent-memory fallback failed: {e}")
